    ...
    <td class="text-right text--secondary">
      <v-tooltip bottom transition="expand-y-transition" open-delay="200">
        <template v-slot:activator="{ on }">
          <v-btn depressed color="transparent" v-on="on" @click.stop="$emit('getEnrollments')">
            {{ enrollments }}
            <v-icon small right>
...
export default {
  ...
  methods: {
    onDrop(event) {
      ...
      const droppedIndividuals = JSON.parse(event.dataTransfer.getData('individuals')).filter(
