<template>
  ...
  >
    ...
    >
      <v-col v-for="individual in savedIndividuals" :key="individual.id" cols="2">
        <individual-card
    ...
    <v-row v-else dense class="space pa-md-2 ma-md-3 align-center justify-center drag-zone">
      <v-icon color="rgba(0,0,0,0.38)" left>
        mdi-lightbulb-on-outline
      </v-icon>
      <p class="mb-0 ml-2 text--disabled">
        <span>
          You can add individuals to your work space to perform actions on them later.
        </span>
        <span>
...
export default {
  ...
  methods: {
    ...
    onDrop(evt) {
      ...
      const droppedIndividuals = JSON.parse(evt.dataTransfer.getData('individuals'));
      const newIndividuals = droppedIndividuals
        .filter(
          (dropped) => !this.savedIndividuals.some((individual) => individual.id === dropped.id),
        )
    ...
    async merge(fromUuids, toUuid) {
      ...
      if (response) {
        this.savedIndividuals = this.updateMergedIndividuals(response.data.merge, fromUuids);
    ...
    async moveAction(fromUuid, toUuid) {
      ...
      try {
        ...
        if (response) {
          this.savedIndividuals = this.updateMergedIndividuals(response.data.moveIdentity, [
            fromUuid,
          ]);
          this.$emit('updateIndividuals');
    ...
    onDrag(event) {
      ...
      if (type === 'move' || type === 'enrollFromOrganization' || types.includes('organization')) {
        return;
      }
